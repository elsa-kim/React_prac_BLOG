...
- bcrypt : 비밀번호를 데이터베이스에 저장할 때 플레인 텍스트로 저장하면 보안상 위험하므로 단방향 해싱 함수 지원해주는 bcrypt 라이브러리 사용해 비밀번호 안전하게 저장
- 모델 메서드 : 모델에서 사용할 수 있는 함수를 의미하며 두 종류 있음
  1. 인스턴스 메서드 : 모델 통해 만든 문서 인스턴스에서 사용할 수 있는 함수
  - 인스턴스 메서드 작성 시 function 키뭐드 사용해 구현해야 함 : 함수 내부에서 this에 접근해야 하는데 화살표 함수 사용 시 this는 문서 인스턴스 가리키지 못함
  2. 스태틱(static) 메서드 : 모델에서 바로 사용할 수 있는 함수

#### 토큰 발습 및 검증

JWT 토큰 : yarn add jsonwebtoken
.env 파일에 JWT 토큰 만들 때 사용할 비밀키 생성 ; JWT_SECRET

- JWT 토큰 서명 만드는 과정에서 사용되므로 외부에 공개되면 안됨

- 사용자가 브라우저에서 토큰 사용할 때는 주로 두가지 방법 사용

  1. 브라우저의 localStorage 또는 sessionStorage에 담아 사용

  - 사용하기 매우 편리하고 구현하기 편함
  - 페이지에 악성 스크립트 삽입하면 쉽게 토큰 탈취 가능(XSS ; Cross Site Scripting)

  2. 브라우저의 쿠키에 담아 사용 => 사용

  - httpOnly 속성 활성화하면 자바스크립트를 통해 쿠키를 조회할 수 없어 악성 스크립트로부터 안전하지만 CSRF(Cross Site Request Forgery) 공격에 취약해 질 수 있음
    - CSRF : 토큰을 쿠키에 담으면 사용자가 서버로 요청 할 때마다 무조건 토큰이 함께 전달되는 점을 이용해 사용자가 모르게 원치 않는 API 요청하게 됨, CSRF 토큰 사용 및 Referer 검증 등의 방식으로 막을 수 있음

- 토큰을 확인한 후 검증하는 작업은 미들웨어 통해 처리
- 토큰 해석
  - iat : 토큰 언제 만들어졌는지 알려주는 값
  - exp : 언제 만료되는지 알려주는 값
